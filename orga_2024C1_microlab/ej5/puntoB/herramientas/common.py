...
def removeLabels(tokens, types):
    ...
    labels = {}
    for t in tokens:
        if len(t) < 2 and t[0] not in ["SET_C", "CLEAR_C", "SET_Z", "CLEAR_Z", "SET_N", "CLEAR_N", "READ_FL", "RET"]:
            return None, None
        if t[0] not in ["SET_C", "CLEAR_C", "SET_Z", "CLEAR_Z", "SET_N", "CLEAR_N", "READ_FL", "RET"] and t[1] == reserveLabel:
